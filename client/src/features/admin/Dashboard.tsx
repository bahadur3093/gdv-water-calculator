...
export default function AdminDashboard() {
  ...
  return (
    <PageShell title="Aqua47 — Admin" navLinks={adminLinks}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-amber-50 rounded-xl border border-amber-200 p-5">
          ...
          <p className="text-2xl font-semibold text-amber-700">{formatCurrency(outstanding)}</p>
          <p className="text-xs text-amber-400 mt-1">{pendingCount} bills pending</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-xs text-gray-400 mb-1">Villas</p>
          <p className="text-2xl font-semibold text-gray-900">{villas.length}</p>
          <p className="text-xs text-gray-400 mt-1">
            {assignedVillas} assigned · {unassignedVillas} unassigned
          </p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-xs text-gray-400 mb-1">Residents</p>
          <p className="text-2xl font-semibold text-gray-900">{residentCount}</p>
          <p className="text-xs text-gray-400 mt-1">
            Rate: {currentRate ? `₹${currentRate.ratePerUnit}/unit` : 'Not set'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">Monthly billing (last 6 months)</h3>
          {months.length === 0 ? (
                ...
                return (
                  <div key={month} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex flex-col justify-end h-32 relative">
                      {/* Total bar */}
                      <div
                        className="w-full bg-brand-100 rounded-t-md absolute bottom-0"
                        style={{ height: `${totalH}%` }}
                      />
                      {/* Paid bar on top */}
                      <div
                        className="w-full bg-brand-600 rounded-t-md absolute bottom-0"
                        style={{ height: `${paidH}%` }}
                      />
                    </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <Link to="/admin/bills" className="text-xs text-brand-600 hover:underline">
            View all
          </Link>
        </div>
                ...
                return (
                  <tr key={bill._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-md border capitalize ${statusColor[bill.status]}`}>
                        {bill.status}
                      </span>
                    </td>
